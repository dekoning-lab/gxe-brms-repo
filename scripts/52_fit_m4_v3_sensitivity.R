#!/usr/bin/env Rscript
############################################################
# 52_fit_m4_v3_sensitivity.R
# M4 (v3): Full hierarchical model — prior sensitivity variant
#
# Same model as 52_fit_m4_v3.R but accepts sigma_scale as argument.
# Used to assess sensitivity to the hyperprior scale choice.
#
# Usage: Rscript scripts/52_fit_m4_v3_sensitivity.R <parenting> <sigma_scale> [start] [end]
#   e.g.: Rscript scripts/52_fit_m4_v3_sensitivity.R sens 0.25 1 100
#
# Output: results/v3_m4_s<NNN>/<parenting>/imp_NNN.rds  (s025 or s100)
#         results/v3_m4_s<NNN>/<parenting>/summary.csv
############################################################
...
if (length(args) < 2) {
  stop("Usage: Rscript 52_fit_m4_v3_sensitivity.R <parenting> <sigma_scale> [start] [end]")
}

parenting_var <- args[1]
sigma_scale   <- as.numeric(args[2])
start_idx <- if (length(args) >= 3) as.integer(args[3]) else 1L
end_idx   <- if (length(args) >= 4) as.integer(args[4]) else 100L

stopifnot(parenting_var %in% c("sens", "cont", "unre"))
stopifnot(sigma_scale > 0 && sigma_scale < 10)
...
# ── Output directory ─────────────────────────────────────
# Label: s025 for 0.25, s100 for 1.00
scale_label <- sprintf("s%03d", round(sigma_scale * 100))
out_dir <- file.path("results", paste0("v3_m4_", scale_label), parenting_var)
