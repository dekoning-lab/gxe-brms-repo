#!/usr/bin/env Rscript
############################################################
# 63_fit_m4_alt5httlpr.R
#
# ALTERNATE-CODING RUN of the production M4 v3 brms fitting script.
# Near-copy of 52_fit_m4_v3.R; only the input file path and output
# directory are changed (production -> alt 5HTTLPR paths). Same
# model, priors, sampler, seeds (20250310+i), LOO computation, and
# summary aggregation. Goal: maximise comparability with the
# production run; only the 5HTTLPR column values differ.
#
# Input:  data_alt_5httlpr/imputed_datasets_for_brms_m100_v2.rds
# Output: results_alt_5httlpr/v3_m4/<parenting>/imp_NNN.rds
#         results_alt_5httlpr/v3_m4/<parenting>/loo_NNN.rds
#         results_alt_5httlpr/v3_m4/<parenting>/summary.csv
#
# (Original docstring follows.)
############################################################
# 52_fit_m4_v3.R  (production)
...
# ── Output directory ─────────────────────────────────────
out_dir <- file.path("results_alt_5httlpr", "v3_m4", parenting_var)
dir.create(out_dir, recursive = TRUE, showWarnings = FALSE)

# ── Load augmented imputed data ─────────────────────────
imputed_data <- readRDS("data_alt_5httlpr/imputed_datasets_for_brms_m100_v2.rds")
