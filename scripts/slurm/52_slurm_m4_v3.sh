...
#SBATCH --job-name=v3_m4
#SBATCH --array=1-300
#SBATCH --output=logs/v3_m4_%A_%a.out
#SBATCH --error=logs/v3_m4_%A_%a.err

# 300 tasks: 3 parenting × 100 imputations (1 per task)
# M4 (v3): Full hierarchical with pass+diff as covariates
# ~45-60 min per fit + LOO
#
# Tasks   1-100: sens
# Tasks 101-200: cont
# Tasks 201-300: unre
...
echo "M4 v3: parenting=${PV}, imp=${IMP} (task ${SLURM_ARRAY_TASK_ID})"
Rscript scripts/52_fit_m4_v3.R ${PV} ${IMP} ${IMP}
