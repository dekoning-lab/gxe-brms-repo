...
#SBATCH --job-name=v3_m1
#SBATCH --array=1-100
#SBATCH --output=logs/v3_m1_%A_%a.out
#SBATCH --error=logs/v3_m1_%A_%a.err

# 100 tasks: 1 imputation each (maximum parallelism)
# M1 (v3): PC1-3 + Sex + infant_age + pass + diff
# ~5 min per fit + LOO
...
echo "M1 v3: imputation ${IMP} (task ${SLURM_ARRAY_TASK_ID})"
Rscript scripts/52_fit_m1_v3.R ${IMP} ${IMP}
